...
test.describe('Admin Reference List', () => {
  test('일자 컬럼/정렬 버튼 렌더', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: '레퍼런스 관리' })).toBeVisible({ timeout: 15_000 });
    await expect(page.getByRole('button', { name: '최근 일자 기준 정렬' })).toBeVisible();
    await expect(page.locator('thead')).toContainText('일자');
  });
});
