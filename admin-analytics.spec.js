...
test.describe('Admin Analytics', () => {
  test('통계분석 페이지 렌더', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: '통계분석' })).toBeVisible({ timeout: 15_000 });
    await expect(page.getByRole('link', { name: 'GA4 열기' })).toBeVisible();
    await expect(page.locator('select')).toBeVisible();
    await expect(page.locator('text=/페이지 로드 중 오류/i')).toHaveCount(0);
  });
});
