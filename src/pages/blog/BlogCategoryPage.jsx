...
export default function BlogCategoryPage() {
  ...
  return (
    <BlogLayout title={currentCategory ? `${currentCategory.name} - 블로그` : '블로그'}>
      <SEOHead
        title={currentCategory ? `${currentCategory.name} | 웨비나스 블로그` : '블로그 | 웨비나스'}
        description={currentCategory?.description || `${currentCategory?.name || '카테고리'} 관련 웨비나스 블로그 글 모음`}
        url={`https://webinars.co.kr/blog/category/${categorySlug}`}
      />
      ...
      <div className="blog-page-header">
        <div className="blog-page-header-inner">
          ...
          <h1 className="blog-title">{currentCategory?.name || '카테고리'}</h1>
          {currentCategory?.description && (
            <p className="blog-subtitle">{currentCategory.description}</p>
          )}
        </div>
      </div>
